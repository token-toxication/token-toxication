...
The **Client Setup** page generates copy-ready configuration for Codex, Claude Code, opencode, Pi, and DeepSeek Harness from eligible routes.
...
The DeepSeek Harness tab merges `llm-pi-ai` provider routes into `$DSH_HOME/settings.yaml` — one route per protocol (Chat, Responses, and Anthropic Messages) — and sets the harness default model. DeepSeek-family Chat models declare `thinkingFormat: deepseek` reasoning dispatch. Recognized GPT-5 and OpenAI o-series Responses models declare only the reasoning levels their exact model ID supports, even when the catalog family is left as `other`. The current Codex catalog — GPT-5.2, GPT-5.4, GPT-5.4 Mini, GPT-5.5, the GPT-5.6 family, and Codex Auto Review — plus the GPT-5.3 Codex variants is covered explicitly; models shared with the public OpenAI API use the safe intersection of both backends. Known multimodal models also declare `input: [text, image]`. Known non-reasoning OpenAI models declare `reasoningEfforts: false`, while unknown models remain undeclared. Every route references the relay key through `TOKEN_TOXICATION_API_KEY`.
